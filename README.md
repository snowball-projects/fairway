...
The current catalog contains eight public 9- and 18-hole courses inside the
`chicago-static-v1` road snapshot. It is dated and incomplete. Results use
static road costs, without traffic, prices, ratings, or live tee-time
availability.
...
`POST /api/rankings`. The process loads one immutable road snapshot, uses modo
to build the origin-to-course travel-time matrix, ranks the catalog, and returns
the result without storing the request.
